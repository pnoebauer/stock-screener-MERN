...
class DataDAO {
  static async injectDB(conn) {
    ...
    try {
      ...
      zips = await conn.db(process.env.MFLIX_TR).collection('zips'); // zips = await conn.db(process.env.MFLIX_TR).collection('movies');
  ...
  static async getZips({
    ...
    let cursor; // const resa = await zips.find({state: 'AL'});
    // const res = await resa.toArray();
    // console.log('running getzips', res, '-----');

    try {
      cursor = await zips.find({
        state: 'AL'
    ...
    const displayCursor = cursor.skip(zipsPerPage * page).limit(zipsPerPage); // const zipsList = await displayCursor.toArray();
    // return {zipsList};

    try {
      const zipsList = await displayCursor.toArray(); // const totalNumZips = page === 0 ? await zips.countDocuments(query) : 0;

      const totalNumZips = page === 0 ? await zips.countDocuments({
        state: 'AL'
      }) : 0;
      return {
        zipsList,
...
class StockDataDAO {
  ...
  static async getPrices({
    ...
    try {
      cursor = await stocks.find(); // console.log(cursor.toArray());
      // .project(project)
      // .sort(sort);
    } catch (e) {
      console.error(`Unable to issue find command, ${e}`);
    ...
    const displayCursor = cursor.skip(pricesPerPage * page).limit(pricesPerPage); // const pricesList = await displayCursor.toArray();
    // return {pricesList};

    try {
      const pricesList = await displayCursor.toArray();
      const totalNumPrices = page === 0 ? await stocks.countDocuments({
        ticker: 'AAPL'
      }) : 0;
      return {
        pricesList,
  ...
  static async insertStockHist() {
    ...
    try {
      let upsertResult = await stocks.updateOne( // this is the "query" portion of the update
      {
        ticker: 'AAPL',
        timestamp: myDate
      }, // this is the update
